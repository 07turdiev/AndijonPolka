      ...
      <div class="header-block">
        ...
        <div class="header-act">
          ...
          <div
            ...
            class="login"
          >
            <span v-if="user" class="first-name">{{ user?.first_name }} {{ user?.sur_name }}</span>
            ...
            <div v-if="user?.first_name && openmodal" class="openmodal">
              ...
              <router-link :to="'/' + this.$i18n.locale + '/profile'">
              {{
                $t('myProfile')
              }}
              </router-link>

              <router-link :to="'/' + this.$i18n.locale + '/tests'">
              {{
                $t('tests')
              }}
              </router-link>
...
export default {
  ...
  methods: {
    async login() {
      await window.open(
        'https://sso.egov.uz/sso/oauth/Authorization.do?response_type=one_code&client_id=study_madaniyhayot_uz&redirect_uri=https://study.madaniyhayot.uz/uz/applications/&scope=study_madaniyhayot_uz&state=IDPW',
        '_self'
      )
    },
    async logout() {
