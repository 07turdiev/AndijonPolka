        ...
        <div class="gids-sg">
            <div class="wrapper">
                <div class="gids-sg-wrapper">
                    <img src="@/assets/images/shutterstock_1022824408.jpg" alt="">
                </div>
                ...
                <div class="gids-sg-block">
                    ...
                    <div v-if="loading" class="gids-sg-uinfos">
                        <div class="gids-sg-upinfos">
                            <img class="gids-avatar" :src="'https://study.madaniyhayot.uz/api/cdn/' + guide?.certificates[guide?.certificates?.length - 1]?.badge_img" alt="" />
                            ...
                            <div class="gids-sg-uinfo">
                                ...
                                <div class="gids-sg-prinfos">
                                    ...
                                    <div v-if="guide?.cerificates?.[0]?.reg_num" class="gids-sg-prinfo">
                                        <span>{{$t('certNumber')}}:</span>
                                        <p>{{guide?.certificates?.[0]?.reg_num || '-'}}</p>
                                    </div>
                                    <div class="gids-sg-prinfo">
                                        <span>{{$t('given_date')}}:</span>
                                        <p>{{ formatDate(guide?.certificates?.[0]?.given_date) }}</p>
                                    ...
                                    <div class="gids-sg-prinfo" style="row-gap:10px;">
                                        ...
                                        <p>{{guide?.certificates?.[0]?.organization['name_'+[$i18n.locale]]}}</p>
                                      <!-- <p v-for="el,index in guide?.regions" :key="index">{{ regions[el-1]?.name?.[$i18n.locale] }},</p> --> 
                                    </div>
                                    <div class="gids-sg-prinfo" style="row-gap:10px;">
                                        <p>“Madaniy hayot media marketing markazi” MCHJ direktori M. Ibroximov tomonidan tasdiqlangan</p>
                                      <!-- <p v-for="el,index in guide?.regions" :key="index">{{ regions[el-1]?.name?.[$i18n.locale] }},</p> --> 
                                    </div>
                                </div>
                                <!-- <div class="qr-code">
                    ...
                    <div v-if="guide?.work_phone_number || guide?.email" class="gids-sg-uinfos ne">
                        <div class="gids-sg-cb">
                            <div v-if="guide?.phone_number" class="gids-sg-ne">
                                <span>{{$t('phone')}}:</span>
                                <a :href="'tel:'+guide?.phone_number">{{guide?.phone_number}}</a> 
                            </div>
                            <div v-if="guide?.email" class="gids-sg-ne">
                                <span>{{$t('mail')}}:</span>
...
export default {
    ...
    mounted(){
        ...
        this.$axios.get(('volunteers/single?id='+this.$route.query.id)).then((res)=>{
            this.guide = res?.data?.result?.userDocument
