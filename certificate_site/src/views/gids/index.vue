      ...
      <div class="wrapper">
        ...
        <div class="gids-table">
          ...
          <div>
            <div class="gids-in">
              <div class="gt-search">
                <input @keypress.enter="searchF" v-model="search_text" :placeholder="placeholder == true ? $t('inputFT') : $t('inputS')
                  " type="text" />
          ...
          <div class="gt-tabs">
            <div class="gt-select">
              ...
              <el-select @change="changeFilter" v-model="filter.guide_type" class="m-2" placeholder="Variant tanlang">
                <el-option v-for="item in guideTypes" :key="item.key" :label="item.label" :value="item.value" />
              </el-select>
            </div>
          </div>
          <div class="gt-tb">
            <div class="table">
              ...
              <div v-else @click="
                $router.push({
                  name: 'volunteers_id',
                  query: { id: el?.data_token },
                })
                " class="tbody" v-for="(el, index) in table" :key="index">
                <div class="td">{{ offset + index + 1 }}</div>
                <div class="td">
                  <img
                    :src="'https://study.madaniyhayot.uz/api/cdn/' + el?.certificates[el?.certificates?.length - 1]?.badge_img"
                    alt="" />
                  <p>
                    {{ el?.first_name }}
                    {{ el?.sur_name }}
                ...
                <div class="td">
                  <span v-if="el?.status == 'active'">{{ $t('active') }}</span>
                  <!-- <p>{{ el?.per_adr }}</p> -->
                </div>
              </div>
            </div>
        ...
        <Pagination :staticCurrentPage="page" @goingToPage="goingToPage" @next="goingToPage" @prev="goingToPage"
          v-if="loading" :pages="pages" />
      </div>
    </div>
  </div>
  <preLoader v-else />
  ...
  data() {
    return {
      ...
      guideTypes: [
        ...
        {
          value: 'akt',
          label: 'Axborot texnologiyalari sohasidagi sertifikatga ega xodimlar',
          key: 'akt',
        }
      ],
    }
  ...
  methods: {
    goingToPage() {
      ...
      this.$axios.get('volunteers', { params: { ...params } }).then((el) => {
        this.table = el?.data?.result?.users
        this.count = el?.data?.result?.count
      })
      this.loading = true
    },
    searchF() {
      ...
      this.$axios.get('volunteers', { params: { ...params } }).then((el) => {
        ...
        this.$router.push({
          query: { ...this.$route.query, search_text: this.search_text },
        })
        this.loading = true
      })
    },
